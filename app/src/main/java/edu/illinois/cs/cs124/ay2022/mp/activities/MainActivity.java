...
public final class MainActivity extends AppCompatActivity
    ...
    androidx.appcompat.widget.SearchView.OnQueryTextListener, MapEventsReceiver {
  ...
  protected void onCreate(final Bundle unused) {
    ...
    GeoPoint defaultCenter = new GeoPoint(40.10986682167534, -88.22831928981661);
  ...
  @Override
  public Consumer<ResultMightThrow<List<Place>>> andThen(final Consumer<? super ResultMightThrow<List<Place>>> after) {
  ...
  public boolean onQueryTextSubmit(final String text) {
    ...
    for (Place place : allPlaces) {
      if (place.getDescription().toLowerCase().contains(text.trim().toLowerCase())) {
        filteredPlaces.add(place);
      }
    }
    if (filteredPlaces.isEmpty()) {
      updateShownPlaces(allPlaces); // Show all places on the map
    } else {
      updateShownPlaces(filteredPlaces); // Show the filtered places on the map
    }
    return false;
  }
  ...
  public boolean onQueryTextChange(final String newText) {
    ...
    List<Place> filteredPlaces = new ArrayList<>();
    for (Place place : allPlaces) {
      if (place.getDescription().toLowerCase().contains(newText.trim().toLowerCase())) {
        filteredPlaces.add(place);
      }
    }
    if (filteredPlaces.isEmpty()) {
  ...
  public boolean longPressHelper(final GeoPoint p) {
    ...
    // Get the latitude and longitude from the GeoPoint and convert them to Strings
    ...
    if (p.getLatitude() >= -90 && p.getLatitude() <= 90 && p.getLongitude() >= -180 && p.getLongitude() <= 180) {
      // Add them as extras to the Intent
